...
async function postHandler(request: NextRequest) {
    try {
        ...
        const result = await llmService.generatePinterestBoardNames(input);

        return NextResponse.json(result);
    } catch (error) {
        console.error("Error in Pinterest Board Name generation API:", error);
        return NextResponse.json(
            { error: "Failed to generate board names" },
            { status: 500 }
        );
    }
}
